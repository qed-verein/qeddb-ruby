...
module ClassMethods
	def default_crud_links(name, options = {})
		operations = [:index, :new, :show, :edit, :destroy]
		operations &= options[:only] if options.has_key? :only
		operations -= options[:except] if options.has_key? :except
