...
automatic_members(group_id, person_id) AS (
	...
	WHERE groups.event_id = registrations.event_id AND
		((groups.kind = 7 AND registrations.organizer) OR
		 (groups.kind = 8 AND registrations.status IN (1, 2)))
	UNION
	SELECT groups.id, people.id
	FROM groups, people
	WHERE (groups.kind = 4 AND people.active AND (CURRENT_TIMESTAMP BETWEEN people.joined AND people.member_until)) OR
		  (groups.kind = 5 AND people.active AND (NOT CURRENT_TIMESTAMP BETWEEN people.joined AND people.member_until OR
				people.joined IS NULL OR people.member_until IS NULL)) OR
		  (groups.kind = 6 AND people.active AND people.newsletter)),
